...
const fixturePath = path.resolve(__dirname, "../../tests/Founders-Agreement_updated-062023.docx");
...
test("uploads a contract from the shared tests fixtures", async ({ page }) => {
  ...
  await page.route("http://localhost:8080/api/v1/contracts", async (route) => {
    ...
    createContractBody = route.request().postDataJSON() as Record<string, unknown>;
  ...
  await page.route(`http://localhost:8080/api/v1/contracts/${contractId}/files`, async (route) => {
    if (route.request().method() !== "POST") {
      await route.fallback();
      return;
    }

    addContractFileBody = route.request().postDataJSON() as Record<string, unknown>;
  ...
  await page.route(`http://localhost:8080/api/v1/contracts/${contractId}`, async (route) => {
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({
  ...
  await page.route(/http:\/\/localhost:8080\/api\/v1\/documents\?.*/, async (route) => {
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({
  ...
  await page.route(`http://localhost:8080/api/v1/documents/${documentId}/text`, async (route) => {
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({
  ...
  await expect(page.locator(".selected-upload-name", { hasText: fixtureName })).toBeVisible();
  ...
  await expect(page.getByText("Contract created. Indexing status will update here.")).toBeVisible();
  await expect(page.locator(".file-name", { hasText: fixtureName })).toBeVisible();
  await expect(page.getByRole("heading", { name: "Contract Text" })).toBeVisible();
  ...
  expect((addContractFileBody?.content_base64 as string).length).toBeGreaterThan(100);
});
